...
module.exports = function(buf) {
	...
	function read2(){
		var val = view.getUint16(position, false);
		position += 2;
		return val;
	}
	function read4(){
		var val = view.getUint32(position, false);
		position += 4;
		return val;
	}
	function read(length, enc){
		if(length == 0) return '';
		var str = new TextDecoder(enc || encoding).decode(buf.slice(position, position + length));
		position += length;
		return str;
	}
	function readGroups(){
	...
	if(position < buf.length)
		obj.data = new TextDecoder(encoding).decode(buf.slice(position));
